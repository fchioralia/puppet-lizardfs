...
)
{
  ...
  if $use_multicast == true {
    ...
    if empty($unicast_addresses) {
        fail('The unicast_addresses parameter is empty, you need to specify the IPs of the servers that form the cluster.')
    }
  ...
  cs_primitive { 'Failover-IP':
    ...
    parameters      => {
        'ip'           => $::lizardfs::master::options[MASTER_HOST],
        'cidr_netmask' => '24' },
